...
import { bridge } from '@/data/mockData';
...
export function Dashboard() {
  const { summary, posture, isDemo, pipelineStage } = useDemo();
  const critical = summary.tinyml_status === 'CRITICAL';
  const warning = summary.tinyml_status !== 'NORMAL';
    ...
    <main className="technical-grid min-h-[calc(100dvh-72px)] px-4 py-6 md:px-8 md:py-7">
      <div className="mx-auto max-w-[1480px] space-y-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" style={{ animationDelay: '100ms' }}>
           <StatusCard label="BRIDGE HEALTH" value={`${summary.bridge_health}%`} detail={summary.bridge_health > 90 ? 'Healthy' : summary.bridge_health > 75 ? 'Monitor' : 'Inspect'} icon={<ShieldCheck size={15} />} tone={critical ? 'red' : warning ? 'amber' : 'green'} />
           <StatusCard label="TINYML STATUS" value={summary.tinyml_status} detail={`Anomaly Score ${summary.anomaly_score.toFixed(2)}`} icon={<BrainCircuit size={15} />} tone={critical ? 'red' : summary.tinyml_status === 'NORMAL' ? 'cyan' : 'amber'} />
           <StatusCard label="LORA NETWORK" value={summary.lora_network_status} detail={`${summary.active_nodes} Nodes · RSSI ${summary.rssi_dbm} dBm`} icon={<Radio size={15} />} tone={critical ? 'red' : summary.lora_network_status === 'CONNECTED' ? 'cyan' : 'amber'} />
          ...
          <section className="panel p-5" data-testid="card-network-posture">
            ...
            <div className="mt-5 flex items-center gap-4"><div className={`flex h-14 w-14 items-center justify-center rounded-full border ${critical ? 'border-rose-300/30 bg-rose-300/[0.06]' : warning ? 'border-amber-300/30 bg-amber-300/[0.06]' : 'border-emerald-300/30 bg-emerald-300/[0.06]'}`}><CheckCircle2 size={26} className={critical ? 'text-rose-300' : warning ? 'text-amber-300' : 'text-emerald-300'} /></div><div><div className="text-lg font-semibold text-slate-100">{postureLabel}</div><p className="mt-1 text-[11px] text-slate-500">{postureDescription}</p></div></div>
             <div className="mt-5 grid grid-cols-2 gap-2">
               <div className="border border-slate-700/70 bg-slate-950/15 p-3"><div className="mono text-[9px] text-slate-500">PACKET DELIVERY</div><div className="mt-1 text-[15px] text-slate-200">{posture.packet_delivery_pct.toFixed(1)}%</div></div>
               <div className="border border-slate-700/70 bg-slate-950/15 p-3"><div className="mono text-[9px] text-slate-500">GATEWAY LATENCY</div><div className="mt-1 text-[15px] text-slate-200">{posture.gateway_latency_ms} ms</div></div>
            </div>
          </section>
        </div>

         <Pipeline activeStage={pipelineStage} />
         <footer className="flex flex-col justify-between gap-2 pb-2 pt-1 text-[10px] text-slate-600 sm:flex-row"><span>GUARDIANBRIDGE LITE · INFRASTRUCTURE SIGNAL INTELLIGENCE</span><span className="mono">{isDemo ? 'DEMO TELEMETRY FABRIC' : 'FIELD TELEMETRY FABRIC'} / API V1</span></footer>
      </div>
    </main>
  );
}